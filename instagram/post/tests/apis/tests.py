...
class PostListViewTest(APILiveServerTestCase):
    ...
    def test_create_post(self):
        """
        Post를 생성하면서 생성된 파일과 소스 파일이 같은지 검사
        :return:
        """
        ...
        # 테스트용 이미지 파일의 경로
        path = os.path.join(settings.STATIC_DIR, 'test', 'cowboy01.jpg')
        print(path)

        with open(path, 'rb') as photo:
            response = self.client.post(
                self.URL_API_POST_LIST,
                {
                    'photo':photo,
                })
        # response가 201인지 확인
        ...
        # 업로드를 시도한 파일 (path경로의 파일) vs 실제 업로드한 파일
        # 즉, 새로 생성된 Post의 photo필드에 있는 파일
